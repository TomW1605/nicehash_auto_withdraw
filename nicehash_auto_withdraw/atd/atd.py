...
def _can_read_file(self, filename):
    ...
    if not os.access(filename, os.R_OK):  # Can't read file
        raise OSError('No permission to use {0}. Try the command `chown 644 {0}` as root.').format(filename)
...
def convert_datetime(dt):
    ...
    if dt.year >= 2038 and (2**64/2)-1 != long(sys.maxsize):
        raise RuntimeWarning('Year >= 2038 detected on system running 32 bit Python. `at` has undefined behavior with years >= 2038. Please make sure your system is 64bit, even if your Python binary isn\'t.')
