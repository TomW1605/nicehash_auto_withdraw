...
class AtJob(object):
    ...
    def __getattr__(self, name):
        ...
        if self.id == 0:
            raise ValueError('You tried to get info about a null (non-existent) job. Set AtJob.id first.')
    ...
    def from_at_stderr(self, stderr):
        ...
        if not match:
            raise NotImplementedError('python-atd doesn\'t seem to support your version of at. Please open an issue. stderr: {0}'.format(stderr))
